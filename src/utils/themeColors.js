...
function localStorageBackup() {
  ...
  if (localStorage.sashBgImage) {
    document.querySelector('body')?.classList.add('transparent-mode')
    const bgImg = localStorage.sashBgImage.split(' ')[0]
...
function bgImage(e) {
  ...
  $('#myonoffswitch5').prop('checked', false)
  $('#myonoffswitch8').prop('checked', false)
  const imgID = e.getAttribute('class')
...
function names() {
  ...
  const colorData = hexToRgba(myVarVal || '#6c5ffc', 0.1)
  document.querySelector('html').style.setProperty('--primary01', colorData)

  const colorData1 = hexToRgba(myVarVal || '#6c5ffc', 0.2)
  document.querySelector('html').style.setProperty('--primary02', colorData1)

  const colorData2 = hexToRgba(myVarVal || '#6c5ffc', 0.3)
  document.querySelector('html').style.setProperty('--primary03', colorData2)

  const colorData3 = hexToRgba(myVarVal || '#6c5ffc', 0.6)
  document.querySelector('html').style.setProperty('--primary06', colorData3)

  const colorData4 = hexToRgba(myVarVal || '#6c5ffc', 0.9)
  document.querySelector('html').style.setProperty('--primary09', colorData4)

  const colorData5 = hexToRgba(myVarVal || '#6c5ffc', 0.05)
