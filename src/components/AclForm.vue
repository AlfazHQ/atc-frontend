<template>
  ...
  <div class="widget-content widget-content-area">
    <h2 class="mb-5">Permissions</h2>
    <div class="form-row ml-3">
      <div v-for="module in modules" :key="module.id" class="form-check col-6">
        <input v-model="module.checked_by_user" class="form-check-input" type="checkbox" :value="module.id" @input="toggleModule(module)">
          ...
          <li v-for="feature in module.features" :key="feature.id">
            <div class="form-row">
              ...
              <div class="form-group col-8">
                <span class="option">
                  <div v-for="permission in feature.permissions" :key="permission.id" class="form-check form-check-inline">
                    <input v-model="permission.checked_by_user" class="form-check-input" type="checkbox" :value="permission.id" @change="togglePermission(module, feature, permission)">
                    <label class="form-check-label">
                      ...
                      <template v-else-if="permission.ability_id === 5">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-download"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" /><polyline points="7 10 12 15 17 10" /><line x1="12" y1="15" x2="12" y2="3" /></svg>                                </template>
                    </label>
                  </div>
                </span>
              </div>
...
export default {
  ...
  mounted() {
    // this.getModules()
    this.$emit('togglePermission', this.handlePermissions())
  },
  methods: {
    // getModules() {
    ...
    // },
    toggleModule(module) {
      module.checked_by_user = !module.checked_by_user
      // console.log('module=>', module.checked_by_user)
      module.features.forEach(feature => {
        feature.permissions.forEach(permission => {
          permission.checked_by_user = !!module.checked_by_user
        })
      })
      this.$emit('togglePermission', this.handlePermissions())
    },
    togglePermission(module, feature, permission) {
      // permission.checked_by_user = !permission.checked_by_user
      ...
      */
      if (permission.ability_id !== 1) {
        const feature_permission = feature.permissions.find(item => item.id === 1)
        if (feature_permission) {
          feature_permission.checked_by_user = true
        }
      }
      /*
      ...
      */
      if (permission.ability_id === 1 && permission.checked_by_user === false) {
        feature.permissions.forEach(permission => {
          permission.checked_by_user = false
        })
      }
      /*
     ...
     */
      ...
      const isCheckedAllAbility = permissions.some(function(permission) { return permission.checked_by_user })
      module.checked_by_user = !!isCheckedAllAbility
      this.$emit('togglePermission', this.handlePermissions())
    },
    handlePermissions() {
      const permissions = []
